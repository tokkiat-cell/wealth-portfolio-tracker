...
// Chat about the user's own portfolio summary, with Anthropic's Claude. Nothing is stored here.
...
export default async function handler(req: Req, res: Res) {
  try {
    ...
    // Whether the Anthropic key is set up, and the models on offer.
    if (req.method === "GET") return send(res, 200, { claude: hasClaudeKey(), claudeModels: CLAUDE_MODELS });
    ...
    const { model, messages, context, search } = parsed.data;
    ...
    return send(res, 200, { reply: a.text, sources: a.sources, model: a.model, searched: a.searched });
  } catch (error) {
